...
impl Advent2016Day20Solver {
    ...
    fn lowest_unblocked(&self) -> u32 {
        self.blacklist.iter()
            .fold(0, |acc, (s, e)| if acc >= *s && acc <= *e { e + 1 } else { acc })
    }

    fn unblocked_count(&self) -> u32 {
        let mut windows: Vec<(u32,u32)> = Vec::new();
        for block in &self.blacklist {
            let start_included = windows.iter().find_position(|w| block.0 >= w.0 && block.0 <= w.1);
            let end_included = windows.iter().find_position(|w| block.1 >= w.0 && block.1 <= w.1);
            match (start_included, end_included) {
                (Some((si, s)), Some((ei, e))) => { if si != ei {
