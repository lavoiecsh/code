...
impl AdventSolver for Advent2016Day22Solver {
    ...
    fn solve_part2(&self) -> usize {
        ...
        while let Some(state) = queue.pop() {
            ...
            state.nexts(&grid)
                .for_each(|s| if !queue.iter().contains(&s) { queue.push(s) });
        }
        0
    }
}
...
impl Debug for State {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("({},{}) ({},{}) -> {} = {}\n{}",
                                 self.zero.0, self.zero.1, self.target.0, self.target.1, self.steps.len(), self.cost(),
                                 self.used.iter().map(|n| n.iter().map(|v| format!("{v:3}")).join("  ")).join("\n")))
    }
}

impl PartialEq<Self> for State {
    fn eq(&self, other: &Self) -> bool {
        self.used.eq(&other.used) &&
            self.target.eq(&other.target) &&
            self.zero.eq(&other.zero)
    }
}
...
impl State {
    fn init(nodes: &[Node]) -> Self {
        Self {
            ...
            zero: nodes.iter().find(|n| n.used == 0).map(|n| (n.x as usize, n.y as usize)).unwrap(),
        }
    }
    ...
    fn cost(&self) -> usize {
        self.steps.len() + self.distance(&self.zero, &(if self.target.0 == 0 { 0 } else { self.target.0 - 1 }, self.target.1)) * 1000 + self.distance(&self.target, &(0, 0)) * 1000000
    }
    ...
    fn distance(&self, from: &Pos, to: &Pos) -> usize {
        ...
        while !distances.contains_key(to) {
            ...
            if current.0 > 0 && !distances.contains_key(&(current.0 - 1, current.1)) && self.used[current.1][current.0 - 1] < 100 {
                distances.insert((current.0 - 1, current.1), next_distance);
                queue.push_back((current.0 - 1, current.1));
            }
            if current.0 < self.used[current.1].len() - 1 && !distances.contains_key(&(current.0 + 1, current.1)) && self.used[current.1][current.0 + 1] < 100 {
                distances.insert((current.0 + 1, current.1), next_distance);
                queue.push_back((current.0 + 1, current.1));
            }
            if current.1 > 0 && !distances.contains_key(&(current.0, current.1 - 1)) && self.used[current.1 - 1][current.0] < 100 {
                distances.insert((current.0, current.1 - 1), next_distance);
                queue.push_back((current.0, current.1 - 1));
            }
            if current.1 < self.used.len() - 1 && !distances.contains_key(&(current.0, current.1 + 1)) && self.used[current.1 + 1][current.0] < 100 {
                distances.insert((current.0, current.1 + 1), next_distance);
                queue.push_back((current.0, current.1 + 1));
            }
        }
    ...
    fn nexts(&self, grid: &Grid) -> impl Iterator<Item=Self> {
        ...
        vec!(
            if self.zero.0 > 0 { Some((Direction::Left, (self.zero.0 - 1, self.zero.1))) } else { None },
            if self.zero.0 < grid.max_x { Some((Direction::Right, (self.zero.0 + 1, self.zero.1))) } else { None },
            if self.zero.1 > 0 { Some((Direction::Up, (self.zero.0, self.zero.1 - 1))) } else { None },
