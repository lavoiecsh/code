...
impl AdventSolver for Advent2018Day23Solver {
    fn solve_part1(&self) -> usize {
        let largest_range = self
            ...
            .iter()
            .max_by(|a, b| {
                a.radius.cmp(&b.radius).then_with(|| {
                    b.pos
                        .distance_from_origin()
                        .cmp(&a.pos.distance_from_origin())
                })
            })
            .unwrap();
        self.nanobots
...
impl Pos {
    ...
    fn distance_to(&self, other: &Pos) -> i64 {
        (self.0.abs_diff(other.0) + self.1.abs_diff(other.1) + self.2.abs_diff(other.2)) as i64
    }
}
...
mod test {
    ...
    fn finds_nanobots_in_range_of_largest_radius() {
        let solver = Advent2018Day23Solver::new(
            "\
pos=<0,0,0>, r=4
pos=<1,0,0>, r=1
pos=<4,0,0>, r=3
