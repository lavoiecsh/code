    ...
    pub fn new(input: String) -> Self {
        ...
        for line in input.lines() {
            if let Some(cap) = re.captures(line) {
                let wi = cap.get(3).unwrap().as_str().split(';').filter(|s| !s.trim().is_empty()).collect_vec();
                units.push(Unit {
                    immune_system,
                    count: parse(cap.get(1)),
                    hit_points: parse(cap.get(2)),
                    weaknesses: wi.iter().find(|wi2| wi2.chars().nth(1).unwrap() == 'w')
                        .unwrap_or(&"")
                        .split(' ')
                        .map(DamageType::try_from)
                        .filter_map(Result::ok)
                        .collect(),
                    immunities: wi.iter().find(|wi2| wi2.chars().nth(1).unwrap() == 'i')
                        .unwrap_or(&"")
                        .split(' ')
                        .map(DamageType::try_from)
                        .filter_map(Result::ok)
...
impl Combat {
    fn new(units: &[Unit], boost: usize) -> Self {
        Self { units: units.iter().map(|u| u.boost(boost)).collect() }
    }
    ...
    fn fight(&mut self) {
        ...
        for attacker in self.units.iter().sorted_by(|a, b| b.effective_power().cmp(&a.effective_power()).then(b.initiative.cmp(&a.initiative))) {
    ...
    fn select_target(&self, attacker: &Unit, selected_attacks: &[(usize, usize)]) -> Option<usize> {
        self.units.iter()
            .filter(|u| u.immune_system != attacker.immune_system && selected_attacks.iter().all(|&(_, d)| u.initiative != d))
    ...
    fn attack(&mut self, attacker_initiative: usize, defender_initiative: usize) {
        let optional_attacker = self.units.iter().find(|u| u.initiative == attacker_initiative);
        if optional_attacker.is_none() { return; }
        let attacker = optional_attacker.unwrap();
        let defender_index = self.units.iter().position(|u| u.initiative == defender_initiative).unwrap();
...
";
    ...
    fn finds_winning_side() {
        let solver = Advent2018Day24Solver::new(String::from(EXAMPLE));
