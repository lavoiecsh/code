...
impl Computer {
    ...
    fn execute(&mut self, instruction: &Instruction) {
        if instruction.condition.fails(self.get_value(&instruction.condition.register)) { return }

        let previous = self.get_value(&instruction.register);
        self.registers.insert(instruction.register.clone(), instruction.new_value(previous));
    }
