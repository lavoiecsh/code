...
impl<'a> Grid<'a> {
    fn init(rules: &'a Vec<Rule>) -> Self {
        Self {
            grid: vec!(vec!(false, true, false), vec!(false, false, true), vec!(true, true, true)),
            rules,
        }
    }
...
impl Rule {
    fn new(from: &str, to: &str) -> Self {
        Self {
            from: from.chars().filter(|c| *c != '/').map(|c| c == '#').collect(),
            to: to.split("/").map(|row| row.chars().map(|c| c == '#').collect()).collect(),
        }
    }
