...
fn fix_weight(programs: &[Program]) -> usize {
    ...
    let mut weights: HashMap<String, usize> = programs
        .iter()
        .filter(|p| p.above.is_empty())
        .map(|p| (p.name.clone(), p.weight))
        .collect();
    while !remaining.is_empty() {
        let position = remaining
            .iter()
            .position(|p| p.above.iter().all(|a| weights.contains_key(a)))
            .unwrap();
        let program = remaining.swap_remove(position);
        let above_weights: Vec<(&String, usize)> = program
            .above
            .iter()
            .map(|a| (a, *weights.get(a).unwrap()))
            .sorted_by_key(|a| a.1)
            .collect();
        if above_weights.iter().map(|a| a.1).all_equal() {
            weights.insert(
                program.name.clone(),
                program.weight + above_weights.iter().map(|a| a.1).sum::<usize>(),
            );
        } else if above_weights[0].1 != above_weights[1].1 {
            let smaller = above_weights[0].1;
            let larger = above_weights[1].1;
            let prev_weight = programs
                .iter()
                .find(|p| &p.name == above_weights[0].0)
                .unwrap()
                .weight;
            ...
            let prev_weight = programs
                .iter()
                .find(|p| &p.name == above_weights[above_weights.len() - 1].0)
                .unwrap()
                .weight;
