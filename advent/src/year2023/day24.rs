...
impl Hailstorm {
        ...
        for i in 0..self.hailstones.len() {
            for j in i + 1..self.hailstones.len() {
                if let Some(intersection) =
                    self.hailstones[i].future_intersection_with(&self.hailstones[j])
                {
    ...
    fn rock_hitting_everything(&self) -> Hailstone {
        ...
        for i in 0..self.hailstones.len() {
            ...
            for j in i + 1..self.hailstones.len() {
                ...
                if hsi.velocity.x == hsj.velocity.x {
                    x_parallels.push(Parallel::new(
                        hsi.position.x - hsj.position.x,
                        hsi.velocity.x,
                    ));
                }
                if hsi.velocity.y == hsj.velocity.y {
                    y_parallels.push(Parallel::new(
                        hsi.position.y - hsj.position.y,
                        hsi.velocity.y,
                    ));
                }
                if hsi.velocity.z == hsj.velocity.z {
                    z_parallels.push(Parallel::new(
                        hsi.position.z - hsj.position.z,
                        hsi.velocity.z,
                    ));
                }
            }
        ...
        let x_velocities = x_parallels
            .iter()
            .map(|p| p.intersecting_velocities())
            .reduce(|a, c| a.intersection(&c).cloned().collect())
            .unwrap();
        let y_velocities = y_parallels
            .iter()
            .map(|p| p.intersecting_velocities())
            .reduce(|a, c| a.intersection(&c).cloned().collect())
            .unwrap();
        let z_velocities = z_parallels
            .iter()
            .map(|p| p.intersecting_velocities())
            .reduce(|a, c| a.intersection(&c).cloned().collect())
            .unwrap();

        if let Some(x_match) = self
            .hailstones
            .iter()
            .find(|hs| x_velocities.iter().contains(&hs.velocity.x))
        {
            let other = self.hailstones.iter().find(|&hs| hs != x_match).unwrap();
            let time =
                (x_match.position.x - other.position.x) / (other.velocity.x - x_match.velocity.x);
            for &vy in &y_velocities {
                ...
                for &vz in &z_velocities {
                    ...
                    let rock = Hailstone::new(
                        Int3 {
                            x: x_match.position.x,
                            y: py,
                            z: pz,
                        },
                        Int3 {
                            x: x_match.velocity.x,
                            y: vy,
                            z: vz,
                        },
                    );
        ...
        } else if let Some(y_match) = self
            .hailstones
            .iter()
            .find(|hs| y_velocities.iter().contains(&hs.velocity.y))
        {
            let other = self.hailstones.iter().find(|&hs| hs != y_match).unwrap();
            let time =
                (y_match.position.y - other.position.y) / (other.velocity.y - y_match.velocity.y);
            for &vx in &x_velocities {
                ...
                for &vz in &z_velocities {
                    ...
                    let rock = Hailstone::new(
                        Int3 {
                            x: px,
                            y: y_match.position.y,
                            z: pz,
                        },
                        Int3 {
                            x: vx,
                            y: y_match.velocity.y,
                            z: vz,
                        },
                    );
                    if rock.hits_all(&self.hailstones) {
        ...
        } else if let Some(z_match) = self
            .hailstones
            .iter()
            .find(|hs| z_velocities.iter().contains(&hs.velocity.z))
        {
            let other = self.hailstones.iter().find(|&hs| hs != z_match).unwrap();
            let time =
                (z_match.position.z - other.position.z) / (other.velocity.z - z_match.velocity.z);
            for &vx in &x_velocities {
                let px = other.position.x + time * (other.velocity.x - vx);
                for &vy in &y_velocities {
                    let py = other.position.y + time * (other.velocity.y - vy);
                    let rock = Hailstone::new(
                        Int3 {
                            x: px,
                            y: py,
                            z: z_match.position.z,
                        },
                        Int3 {
                            x: vx,
                            y: vy,
                            z: z_match.velocity.z,
                        },
                    );
                    if rock.hits_all(&self.hailstones) {
                        return rock;
...
";
    ...
    fn counts_future_hailstone_crossings() {
        let solver = Advent2023Day24Solver::new(EXAMPLE);
        let solution = solver.hailstorm.crossing_count(Range2 {
            x: 7f64..27f64,
            y: 7f64..27f64,
        });
