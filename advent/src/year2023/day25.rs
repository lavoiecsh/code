...
impl AdventSolver for Advent2023Day25Solver {
    fn solve_part1(&self) -> usize {
        ...
        let most_visited = visited_counts.iter().sorted_by_key(|(_,v)| *v).rev().take(100).collect_vec();
...
impl Graph {
    fn new(connections: &Vec<(String, String)>) -> Self {
        let mut nodes: Vec<Node> = connections.iter()
    ...
    fn visited_edges(&self, id: usize) -> Vec<(usize, usize)> {
        ...
        while touched.len() != self.nodes.len() {
            ...
            while let Some(current) = queue.pop() {
                let untouched_links = self.nodes[current].links.iter().filter(|l| !touched.contains(l)).collect_vec();
    ...
    fn groups(&self, broken_links: &Vec<(usize, usize)>) -> Vec<usize> {
        ...
        while let Some(next_group) = group.iter().position(|g| !created_groups.contains(g)) {
            let mut queue: VecDeque<usize> = self.nodes[next_group].links.iter()
                .filter(|&&l| !broken_links.contains(&(next_group, l)) && !broken_links.contains(&(l, next_group)))
                .cloned()
                .collect();
            while let Some(current) = queue.pop_front() {
                if group[current] == next_group { continue; }
                group[current] = next_group;
                queue.extend(self.nodes[current].links.iter()
                    .filter(|&&l| !broken_links.contains(&(current, l)) && !broken_links.contains(&(l, current))));
            }
...
impl Node {
    fn new(id: usize, name: String) -> Self {
        Self { id, name, links: vec!() }
    }
}

impl Debug for Node {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{:4} ({:3}) = {:2} -> {}",
                                 self.id, self.name, self.links.len(), self.links.iter().join(", ")))
    }
}

#[cfg(test)]
fn test_solver_1() -> Advent2023Day25Solver {
    Advent2023Day25Solver::new(String::from("\
jqt: rhn xhk nvd
rsh: frs pzl lsr
xhk: hfx
cmg: qnr nvd lhk bvb
